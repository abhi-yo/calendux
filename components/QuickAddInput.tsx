...
interface QuickAddInputProps {
  onCreateEvent: (event: ParsedEvent) => Promise<void> | void;
  open?: boolean;
        ...
        <form onSubmit={handleSubmit}>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <span>{format(new Date(preview.start), "EEE, MMM d")}</span>
                    {!preview.allDay && (
                      <>
                        <span>•</span>
                        <span>
                          {format(new Date(preview.start), "h:mm a")} -{" "}
                          {format(new Date(preview.end), "h:mm a")}
                        </span>
                      </>
                    )}
                    {preview.allDay && (
          ...
          {error && !loading && input && (
            <div className="p-4 text-sm text-muted-foreground">
              <span className="text-orange-500">⚠️</span> {error}. Try being
              more specific.
            </div>
          )}
